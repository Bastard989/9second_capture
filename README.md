...
После завершения записи UI открывает окно ввода имени MP3 (например, `Интервью_Иванов.mp3`). Это имя используется для экспортной копии файла, а канонический `meeting_audio.mp3` всё равно остаётся локально внутри папки встречи и может быть использован повторно.

По умолчанию UI работает в режиме `record-first`: во время записи показывает только состояние захвата (System/Mic/диагностика) и понятный live-индикатор здоровья записи, а итоговый `raw/clean` текст строится только после `Стоп` по финальному MP3. Live-транскрипт в интерфейсе отключён, чтобы снизить нагрузку и повысить стабильность.

UI построен по режимам: `Драйвер`, `Экран+аудио`, `API/файл`, `Ссылка (quick)`. При выборе режима лишние блоки и кнопки автоматически становятся неактивными (серые), а рядом с настройками есть `?`-подсказки простым языком.
...
- участвует в пост-обработке только после завершения записи, когда есть цельный MP3 и стабильный контекст встречи.

Почему это важно для вашей цели: LLM превращает “сырой поток реплик” в унифицированный формат оценки кандидата, где есть критерии, сигналы риска, confidence и явный статус решения. За счёт этого любой участник процесса найма, который не присутствовал на интервью, читает не хаотичный текст, а сопоставимый отчёт.
...
### Повторное использование MP3
После каждой завершённой встречи локально сохраняется канонический файл `meeting_audio.mp3` внутри папки этой встречи. Это значит, что запись можно использовать повторно в любой момент: заново сгенерировать транскрипт/отчёт, выгрузить в другое место через UI или передать в внешний пайплайн аналитики.

Экспортное имя MP3, которое пользователь вводит после `Стоп`, влияет только на экспортную копию файла. Внутренний файл в хранилище встречи остаётся стабильным (`meeting_audio.mp3`) для надёжной автоматизации.
